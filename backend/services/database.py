import os
import json
import firebase_admin
from firebase_admin import credentials, firestore
from dotenv import load_dotenv

load_dotenv()

firebase_json = os.environ.get("FIREBASE_SERVICE_ACCOUNT_JSON")
if firebase_json:
    cred = credentials.Certificate(json.loads(firebase_json))
else:
    cred = credentials.Certificate("firebase-service-account.json")
firebase_admin.initialize_app(cred)
db = firestore.client()

def get_knowledge_base():
    docs = db.collection("knowledge_base").where("active", "==", True).stream()
    return [doc.to_dict() for doc in docs]

def get_knowledge_base_by_topic(topic):
    docs = db.collection("knowledge_base").where("topic", "==", topic).where("active", "==", True).stream()
    return [doc.to_dict() for doc in docs]

def log_chat(question, answer, topic=None):
    db.collection("chat_logs").add({"question": question, "answer": answer, "topic": topic or "general", "timestamp": firestore.SERVER_TIMESTAMP})

def get_all_entries():
    docs = db.collection("knowledge_base").stream()
    return [{"id": doc.id, **doc.to_dict()} for doc in docs]

def add_entry(topic, question, answer, keywords=""):
    doc_ref = db.collection("knowledge_base").add({"topic": topic, "question": question, "answer": answer, "keywords": keywords, "active": True, "source": "admin"})
    return doc_ref[1].id

def update_entry(entry_id, data):
    db.collection("knowledge_base").document(entry_id).update(data)

def delete_entry(entry_id):
    db.collection("knowledge_base").document(entry_id).update({"active": False})

def get_chat_logs(limit=50):
    docs = db.collection("chat_logs").order_by("timestamp", direction=firestore.Query.DESCENDING).limit(limit).stream()
    return [{"id": doc.id, **doc.to_dict()} for doc in docs]

def get_topic_stats():
    docs = db.collection("chat_logs").stream()
    stats = {}
    for doc in docs:
        topic = doc.to_dict().get("topic", "general")
        stats[topic] = stats.get(topic, 0) + 1
    return stats
