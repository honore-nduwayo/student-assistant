...
ROOT CAUSE OF THE OUTAGE (documented here so it never happens again):
  The GEMINI_MODEL environment variable on Render was set to "gemini-3-flash".
  That model does not exist. Every code-level fix was silently overridden by
  that env var, so the bot kept failing even after code changes.

  WHAT THIS FILE DOES DIFFERENTLY:
  1. _resolve_model() checks the env var against a BANNED list and refuses to
     use it if it is a known-bad name — falling back to CONFIRMED_MODEL.
  2. CONFIRMED_MODEL = "gemini-3.1-flash-lite-preview" (the model that works,
     same as refresh_knowledge.py which has been running correctly).
  3. The generate_content() call now uses thinking_config — exactly matching
     the pattern in refresh_knowledge.py.
  4. Every startup prints the resolved model name so it is visible in Render logs.
  5. KB Direct-Answer Fallback is the strict last resort when ALL API calls fail.
"""
...
# ── MODEL CONFIGURATION ───────────────────────────────────────────────────────
# Confirmed working — same default as refresh_knowledge.py.
# DO NOT change this to "gemini-3-flash" — that model does not exist.
CONFIRMED_MODEL = "gemini-3.1-flash-lite-preview"

# Fallback chain tried in order if primary returns 404.
# "gemini-2.0-flash-lite" is intentionally excluded (deprecated/unsupported).
...
# Models known to be invalid — env var is ignored if it contains one of these.
BANNED_MODELS = {"gemini-3-flash", "gemini-3.0-flash", "gemini-2.0-flash-lite"}


def _resolve_model() -> str:
    """
    Returns the model to use.
    Reads GEMINI_MODEL env var; rejects it if it is in BANNED_MODELS.
    Always prints the resolved name — visible in Render logs on every request.
    """
    env_val = os.getenv("GEMINI_MODEL", "").strip()
    if env_val and env_val not in BANNED_MODELS:
        resolved = env_val
    else:
        if env_val in BANNED_MODELS:
            print(
                f"[ModelResolver] ⚠️  GEMINI_MODEL='{env_val}' is BANNED "
                f"(model does not exist in Gemini API). "
                f"Using confirmed model: {CONFIRMED_MODEL}"
            )
        resolved = CONFIRMED_MODEL
    print(f"[ModelResolver] Active model: {resolved}")
    return resolved
...
_connection_tracker = {
    ...
    "model_attempts":      {},
    "error_types":         {},
    "recent_failures":     [],   # last 10 entries
}
...
def _track_attempt(model: str, key_index: int):
    ...
    _connection_tracker["model_attempts"].setdefault(
...
def _track_failure(model: str, key_index: int, error_type: str, detail: str):
    _connection_tracker["total_failures"] += 1
    _connection_tracker["last_failure"]        = _now_iso()
    _connection_tracker["last_failure_reason"] = f"{error_type}: {detail[:120]}"
    _connection_tracker["error_types"][error_type] = \
        _connection_tracker["error_types"].get(error_type, 0) + 1
    _connection_tracker["model_attempts"].setdefault(
        model, {"attempts": 0, "successes": 0, "failures": 0}
    )["failures"] += 1
    rec = {"ts": _now_iso(), "model": model, "key_index": key_index,
           "error_type": error_type, "detail": detail[:200]}
    _connection_tracker["recent_failures"].append(rec)
    if len(_connection_tracker["recent_failures"]) > 10:
        _connection_tracker["recent_failures"].pop(0)
    print(f"[ConnTracker] ❌ FAIL | model={model} | key={key_index} | {error_type} | {detail[:120]}")
...
# ── KEY DIAGNOSTIC ────────────────────────────────────────────────────────────
def test_all_keys():
    """Tests every configured key. Prints results to stdout (visible in Render logs)."""
    keys  = get_api_keys()
...
def _detect_current_period(now: datetime, cal: dict) -> str:
    ...
    def in_range(s, e):
        try:
            return (datetime.strptime(cal[s], "%Y-%m-%d").date()
                    <= today <=
                    datetime.strptime(cal[e], "%Y-%m-%d").date())
        except Exception:
            return False
...
# ── KB DIRECT-ANSWER FALLBACK (LAST RESORT) ───────────────────────────────────
_STOP_WORDS = {
...
def _search_kb_directly(question: str, knowledge_base: list):
    ...
    for entry in knowledge_base:
        ...
        combined = set(f"{q_text} {a_text} {kw_text}".split()) - _STOP_WORDS
        score    = len(q_words & combined)
        if any(w in q_text for w in q_words if len(w) > 3):
            score += 3
        ...
        if score > best_score:
            best_score = score
            best_entry = entry
    return best_entry if best_entry and best_score >= 2 else None


def _format_kb_fallback(entry: dict) -> str:
    answer = entry.get("answer", "")
    topic  = entry.get("topic", "general").capitalize()
    return (
        f"⚠️ *AI service temporarily unavailable — this answer is from the Knowledge Base.*\n\n"
        f"**{topic}**\n\n"
        f"{answer}\n\n"
        f"---\n"
        f"*For more details, contact:*\n"
        f"• **Registry:** registry@acity.edu.gh | +233 302 909 838\n"
        f"• **Portal:** https://acityplus.acity.edu.gh\n\n"
...
Kai:"""
    ...
    for m in MODEL_FALLBACK_CHAIN:
        if m not in models_to_try and m not in BANNED_MODELS:
            models_to_try.append(m)

    print(f"[API] {len(keys)} key(s) | model chain: {models_to_try}")

    if not keys:
        print("[API] ❌ No API keys configured.")
        return "Configuration error: no API keys found. Contact registry@acity.edu.gh."
    ...
    for model in models_to_try:
        ...
        for i, key in enumerate(keys, 1):
            ...
            try:
                client   = genai.Client(api_key=key)
                # Using thinking_config — same as refresh_knowledge.py which works
                ...
                if "404" in err or "NOT_FOUND" in err:
                    _track_failure(model, i, "MODEL_NOT_FOUND", err)
                    got_model_404 = True
                    break   # model doesn't exist — skip remaining keys, try next model
                elif "429" in err or "RESOURCE_EXHAUSTED" in err:
                    _track_failure(model, i, "RATE_LIMITED", err)
                    continue
                elif "401" in err or "UNAUTHENTICATED" in err:
                    _track_failure(model, i, "INVALID_KEY", err)
                    continue
                elif "403" in err or "PERMISSION_DENIED" in err:
                    _track_failure(model, i, "PERMISSION_DENIED", err)
                    continue
                elif "FAILED_PRECONDITION" in err:
                    _track_failure(model, i, "FAILED_PRECONDITION", err)
                    continue
                else:
                    _track_failure(model, i, "UNKNOWN_ERROR", err)
                    continue

        if got_model_404:
            print(f"[API] Model '{model}' returned 404 — trying next in chain.")

    # ── All models × all keys exhausted ──────────────────────────────────────
    print(f"[API] ❌ Everything failed. Last: {_connection_tracker['last_failure_reason']}")

    # ── KB fallback — strictly last resort, clearly labelled ─────────────────
    print("[KB Fallback] Searching knowledge base directly...")
    kb_entry = _search_kb_directly(question, knowledge_base)
    if kb_entry:
        _connection_tracker["kb_fallback_used"] += 1
        print(f"[KB Fallback] ✅ Match: '{kb_entry.get('question','')[:60]}'")
        return _format_kb_fallback(kb_entry)

    print("[KB Fallback] No confident match found.")
    return (
        "⚠️ I'm experiencing a service disruption and couldn't find a matching answer in the knowledge base. "
