    ...
    if (part.match(/^https?:\/\//)) {
      return (
        <a key={i} href={part} target="_blank" rel="noreferrer"
          style={{ color: "#1a56db", textDecoration: "underline", wordBreak: "break-all" }}>
          {part}
        </a>
      );
    }
    ...
    if (line.trim().startsWith("* ") || line.trim().startsWith("- ")) {
      ...
      return (
        <div key={i} style={{ display: "flex", gap: "8px", marginBottom: "4px" }}>
          <span style={{ color: "#003087", fontWeight: "bold", flexShrink: 0 }}>•</span>
          <span>{renderInlineLinks(content)}</span>
        </div>
      );
    }
    if (/^\d+\./.test(line.trim())) {
      ...
      return (
        <div key={i} style={{ display: "flex", gap: "8px", marginBottom: "4px" }}>
          <span style={{ color: "#003087", fontWeight: "bold", minWidth: "20px", flexShrink: 0 }}>{num}.</span>
          <span>{renderInlineLinks(content)}</span>
        </div>
      );
    }
...
const SUGGESTED = [
  ...
  "What are the BSc Computer Science fees?",
  "When do Semester 1 exams start?",
  "How do I apply for hostel?",
  "What programmes does ACity offer?",
];
...
export default function App() {
  const [messages, setMessages] = useState([
    {
      role: "bot",
      text: "👋 Welcome! I'm the **ACity Student Assistant**.\n\nI can help you with:\n* Registration & academic calendar\n* Fees & payment methods\n* Course enrollment\n* Exams & results\n* Hostel & accommodation\n\nWhat would you like to know?",
    },
  ]);
        ...
        <div style={s.header}>
          ...
          <div style={s.headerInfo}>
            ...
            <div style={s.headerSub}>
              <span style={s.dot} /> Online · Academic City University College
            </div>
          </div>
        </div>
        ...
        <div style={s.messages}>
          ...
          {loading && (
            <div style={s.botRow}>
              ...
              <div style={{ ...s.botBubble, color: "#94a3b8", fontStyle: "italic" }}>
                Thinking...
              </div>
            </div>
          )}
...
const s = {
  page: {
    minHeight: "100vh",
    background: "linear-gradient(135deg, #001f5b 0%, #003087 50%, #0050c8 100%)",
    display: "flex", alignItems: "center", justifyContent: "center",
    fontFamily: "'Segoe UI', Arial, sans-serif", padding: "16px",
  },
  container: {
    width: "100%", maxWidth: "720px", height: "92vh",
  ...
  avatar: {
    fontSize: "20px", background: "#fff", width: "36px", height: "36px",
    borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center",
    boxShadow: "0 2px 8px rgba(0,0,0,0.1)", flexShrink: 0,
  },
  botBubble: {
    background: "#fff", borderRadius: "4px 16px 16px 16px",
    padding: "12px 16px", maxWidth: "82%", fontSize: "14px", lineHeight: "1.7",
    boxShadow: "0 2px 8px rgba(0,0,0,0.08)", color: "#1e293b",
  },
  userBubble: {
    background: "linear-gradient(135deg, #003087, #0050c8)", color: "#fff",
    borderRadius: "16px 4px 16px 16px", padding: "12px 16px",
    maxWidth: "78%", fontSize: "14px", lineHeight: "1.7",
    boxShadow: "0 2px 8px rgba(0,48,135,0.3)",
  },
  suggestions: {
    padding: "8px 16px 12px", display: "flex", flexWrap: "wrap", gap: "8px",
    background: "#f8fafc", borderTop: "1px solid #e2e8f0",
  },
  chip: {
    background: "#fff", border: "1px solid #cbd5e1", borderRadius: "20px",
    padding: "6px 14px", fontSize: "12px", color: "#003087",
    cursor: "pointer", fontWeight: "500",
  },
  inputArea: {
    display: "flex", padding: "12px 16px", borderTop: "1px solid #e2e8f0",
    gap: "10px", background: "#fff", alignItems: "flex-end",
  },
  input: {
    flex: 1, border: "2px solid #e2e8f0", borderRadius: "12px",
    padding: "10px 14px", fontSize: "14px", resize: "none",
    outline: "none", fontFamily: "inherit", color: "#1e293b", lineHeight: "1.5",
  },
  sendBtn: {
    background: "linear-gradient(135deg, #003087, #0050c8)", color: "#fff",
